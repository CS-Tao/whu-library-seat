      ...
      <div style="margin:auto;text-align:center;">
        <el-form-item label="日期">
          <el-select
            ...
            placeholder="请选择日期"
            class="input"
            @focus="$store.dispatch('updateFreeDates')"
            @change="updateSeatsStatus()"
          >
            <el-option
              key="0"
              :label="'今天' + ' - ' + getWeekDay(freeDates[0])"
              :value="freeDates[0]"
            >
              <span>{{'今天 / ' + freeDates[0] + ' / ' + getWeekDay(freeDates[0])}}</span>
            </el-option>
            <el-option
              key="1"
              :label="'明天' + ' - ' + getWeekDay(freeDates[1])"
              :value="freeDates[1]"
            >
              <span>{{'明天 / ' + freeDates[1] + ' / ' + getWeekDay(freeDates[1])}}</span>
            </el-option>
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="场馆">
          <el-select
            ...
            placeholder="请选择场馆"
            no-data-text="数据加载失败，请重新登录"
            class="input"
            @change="libraryChanged()"
          >
            <el-option
              v-for="library in libraryInfo.buildings"
              :key="library[0]"
              :label="library[1]"
              :value="library[0]"
            ></el-option>
        ...
        <el-form-item label="房间">
          <el-select
            v-model="form.room"
            placeholder="请选择房间"
            no-data-text="数据加载失败，请选择场馆或重新登录"
            class="input"
            @change="roomChanged()"
          >
            <el-option
              v-for="room in singleLibRooms"
              :key="room[0]"
              :label="room[1]"
              :value="room[0]"
            >
              ...
              <span
                v-if="getRoomDetail(room[0])"
                :class=" getRoomDetail(room[0]).free > 0 ? 'room-option-free-green' : 'room-option-free'"
              >{{ getRoomDetail(room[0]).free }}&nbsp;座空闲</span>
            </el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="位置" :inline="true" style="flex-col">
          <el-select
            ...
            :placeholder="seatsForSelectIsAvailable!==null?(`共 ${seatsForSelectIsAvailable.length} 座可约`):seatsForSelect.length + ' 个座位'"
          ...
          >
            <el-option-group :key="'seat-group-1'" label="可选功能">
              <el-option :key="'seat-group-1-0'" :value="-1" label="监控模式">
                ...
                <span class="seat-option-right">
                  ...
                  >
                    <path
                      d="M750.81728 870.23616 273.18272 870.23616c-18.80064 0-34.11968 34.11968-34.11968 34.11968l0 34.11968 545.87392 0 0-34.11968C784.93696 885.5552 769.61792 870.23616 750.81728 870.23616z"
                    />
                    <path
                      d="M853.17632 85.52448 170.82368 85.52448c-56.50432 0-102.35904 45.85472-102.35904 102.35904l0 477.65504c0 56.50432 45.85472 102.35904 102.35904 102.35904l238.81728 0 0 34.11968c0 18.80064 15.31904 34.11968 34.11968 34.11968l136.47872 0c18.80064 0 34.11968-17.59232 34.11968-34.11968 0-7.12704 0-20.84864 0-34.11968l238.81728 0c56.50432 0 102.35904-45.83424 102.35904-102.35904L955.53536 187.88352C955.53536 131.3792 909.68064 85.52448 853.17632 85.52448zM170.82368 631.41888c-18.80064 0-34.11968-15.31904-34.11968-34.11968L136.704 187.88352c0-18.80064 15.31904-34.11968 34.11968-34.11968l682.35264 0c18.80064 0 34.11968 15.31904 34.11968 34.11968l0 409.41568c0 18.80064-15.31904 34.11968-34.11968 34.11968L170.82368 631.41888zM853.13536 735.0272c-19.49696 0-35.30752-15.83104-35.30752-35.38944 0-19.51744 15.83104-35.38944 35.30752-35.38944 19.5584 0 35.38944 15.85152 35.38944 35.38944C888.5248 719.19616 872.69376 735.0272 853.13536 735.0272z"
                    />
                  </svg>
                  <svg
                    class="seat-flag"
                    :class="{'seat-flag-power':form.battery}"
                    version="1.1"
                    xmlns="http://www.w3.org/2000/svg"
                    xmlns:xlink="http://www.w3.org/1999/xlink"
                    x="0px"
                    y="0px"
                    viewBox="0 0 313.585 313.585"
                    style="enable-background:new 0 0 313.585 313.585;"
                    xml:space="preserve"
                  >
                    <g>
                      <path
                  ...
                  <svg
                    class="seat-flag"
                    :class="{'seat-flag-window':form.sun}"
                    version="1.1"
                    xmlns="http://www.w3.org/2000/svg"
                    xmlns:xlink="http://www.w3.org/1999/xlink"
                    x="0px"
                    ...
                    style="enable-background:new 0 0 511.34 511.34;"
                    xml:space="preserve"
                  >
                    <g>
                      <path
            ...
            <el-option-group
              :key="'seat-group-2'"
              :label="seatsForSelectIsAvailable!==null ? ('共 ' + seatsForSelectIsAvailable.length + ' 座可约') : '数据加载中...'"
            >
              <el-option
                v-for="n in seatsForSelect.length"
                :key="n-1"
                :label="seatsForSelect[n-1].name"
                :value="seatsForSelect[n-1].id"
              >
                <span
                  class="seat-option-left"
                  :style="{color: getSeatColor(seatsForSelect[n-1].id)}"
                >{{ seatsForSelect[n-1].name }}</span>
                <span class="seat-option-right">
                  <svg
                    class="seat-flag"
                    :class="{'seat-flag-computer':seatsForSelect[n-1].computer}"
                    viewBox="0 0 1024 1024"
                    version="1.1"
                    xmlns="http://www.w3.org/2000/svg"
                    xmlns:xlink="http://www.w3.org/1999/xlink"
                  ...
                  <svg
                    class="seat-flag"
                    :class="{'seat-flag-power':seatsForSelect[n-1].power}"
                    version="1.1"
                    xmlns="http://www.w3.org/2000/svg"
                    xmlns:xlink="http://www.w3.org/1999/xlink"
                    x="0px"
                  ...
                  <svg
                    class="seat-flag"
                    :class="{'seat-flag-window':seatsForSelect[n-1].window}"
                    version="1.1"
                    xmlns="http://www.w3.org/2000/svg"
                    xmlns:xlink="http://www.w3.org/1999/xlink"
                    x="0px"
          ...
          <el-button class="toggle-button" @click="sunButtonClicked()">
            <svg
              class="icon"
              :class="form.sun?'sun-checked':'sun-unchecked'"
              version="1.1"
              xmlns="http://www.w3.org/2000/svg"
              xmlns:xlink="http://www.w3.org/1999/xlink"
              x="0px"
          ...
          <el-button class="toggle-button" @click="chargerButtonClicked()">
            <svg
              class="icon"
              :class="form.battery?'charger-checked':'charger-unchecked'"
              version="1.1"
              xmlns="http://www.w3.org/2000/svg"
              xmlns:xlink="http://www.w3.org/1999/xlink"
              x="0px"
...
export default {
  ...
  computed: {
    ...
    seatSelectNoDataMessage () {
      ...
      if (!this.seats.length || this.seats.length === 0) {
        ...
        message = `没有筛选到${
          this.form.battery && this.form.sun
            ? '有电源且靠窗'
            : this.form.battery
              ? '有电源'
              : '靠窗'
  ...
  methods: {
    ...
    libraryChanged () {
      ...
      // 提取缓存
      if (
        this.seatInfo.room !== null &&
        this.seatInfo.library === this.form.library
      ) {
      ...
      // 异步更新房间内可用座位数
      libraryRestApi
        .RoomStats(this.form.library, this.userToken)
        .then((response) => {
    ...
    roomChanged () {
      ...
      if (this.form.room === null || this.form.date === null) {
        return
      }
      libraryRestApi
        .LayoutByDate(this.form.room, this.form.date, this.userToken)
        .then((response) => {
          if (response.data.status === 'success') {
    ...
    filterSeats () {
      this.seatsForSelect = this.seats
        .filter((item) => {
          return (
            item.type === 'seat' &&
            (this.form.battery ? item.power : true) &&
            (this.form.sun ? item.window : true)
          )
        })
        .sort((x, y) => {
    ...
    mainButtonClicked () {
      ...
      if (
        this.freeDates[0] !== today ||
        this.freeDates[1] !== tomorrow ||
        !this.freeDates.includes(this.form.date)
      ) {
    ...
    checkLibraryIsOpen () {
      if (
        this.settingInfo.checkOpenEnable &&
        this.form.date === this.freeDates[1]
      ) {
        ...
        this.openCheckMessaggeHandle = this.$message({
          ...
          duration: '0',
          showClose: true,
          message: `正在 (提前) 检测是否可以预约明天的座位，最多检测 ${
            this.maxOpenCheckCount
          } 次，最长耗时 ${
            (this.openCheckInterval * this.maxOpenCheckCount) / 1000
          }s，请稍后...`
        })
        var checkOpenTimerId = setInterval(() => {
          ...
          if (this.openCheckCount > this.maxOpenCheckCount) {
            ...
            this.windowsNotification(
              `预约失败`,
              `系统还未开放该时间段 (可能是您的系统时间有误)`
            )
          ...
          libraryRestApi.FreeFilters(this.userToken).then((response) => {
            if (response.data.status === 'success') {
              if (
                ...
                response.data.data.dates.length > 1
              ) {
                this.openCheckCount = 0
                if (this.openCheckMessaggeHandle) {
                  this.openCheckMessaggeHandle.close()
    ...
    loginAndReserveSeat () {
      ...
      this.seatsSearched = null
      libraryRestApi
        .Login(this.userAccount, this.userPasswd)
        .then((response) => {
          if (response.data.status === 'success') {
            ...
            if (response.data.status === 'success') {
              ...
              if (reservation && reservation.length > 0) {
                if (reservation[0].stat === 'RESERVE') {
                  // 取消当前预约
                  libraryRestApi
                    .Cancel(reservation[0].id, this.userToken)
                    .then((response) => {
                      if (response.data.status === 'success') {
                        this.$message({
                          type: 'success',
                        ...
                        usageApi.grabState(
                          ...
                          false,
                          13,
                          `取消当前预约失败：${response.data.message}`
                        )
                      }
                      this.reserveSeat(
                        beginTime,
                    ...
                    .then((response) => {
                      if (response.data.status === 'success') {
                        ...
                        usageApi.grabState(
                          ...
                          false,
                          15,
                          `终止当前使用失败：${response.data.message}`
                        )
                      }
                      this.reserveSeat(
                        beginTime,
              ...
              usageApi.grabState(
                ...
                false,
                18,
                `获取预约历史失败：${response.data.message}`
              )
              this.reserveSeat(
                beginTime,
                endTime,
        ...
        .then((response) => {
          if (response.data.status === 'success') {
            ...
            this.$notify({
              ...
              message:
                '<el-card shadow="never" style="line-height: 30px;">ID：' +
                response.data.data.id +
                '<br/>凭证：' +
                response.data.data.receipt +
                '<br/>日期：' +
                response.data.data.onDate +
                '<br/>时间：' +
                response.data.data.begin +
                ' - ' +
            ...
            this.windowsNotification(
              `预约成功 ${response.data.data.onDate} - ${response.data.data.begin} - ${response.data.data.end}`,
            ...
            if (response.data.code === 1 || response.data.code === '1') {
              ...
              this.grabCount += 1
              var haveReservation =
                response.data.message.indexOf('已有') !== -1 &&
              ...
              if (this.grabCount >= maxGrabCount) {
                ...
                this.windowsNotification(
                  '抢座失败',
                  `达到抢座尝试上限(${maxGrabCount})，结束抢座`
                )
                usageApi.grabState(
                  this.userAccount,
                  false,
                  7,
                  `抢座失败：达到抢座尝试上限(${maxGrabCount})，结束抢座`
                )
              } else if (newSeatId === -1) {
                this.$store.dispatch('updateTimer', 'fail')
                this.$message({
                ...
                usageApi.grabState(
                  ...
                  false,
                  8,
                  `抢座失败：该房间在指定的时间段内没有空闲位置(${date} ${beginTime}-${endTime})`
                )
              ...
              this.windowsNotification(
                '抢座失败',
                response.data.message ? response.data.message : emptyMessage
              )
              usageApi.grabState(
                this.userAccount,
                false,
    ...
    getNewSeatNum () {
      ...
      for (
        let distance = 1;
        distance < seatInTheRoomBetter.length;
        distance++
      ) {
        if (
          seedIndex - distance >= 0 &&
          !!seatInTheRoomBetter[seedIndex - distance].id &&
          !triedSeatIds.includes(seatInTheRoomBetter[seedIndex - distance].id)
        ) {
          return seatInTheRoomBetter[seedIndex - distance].id
        } else if (
          seedIndex + distance < seatInTheRoomBetter.length &&
          !!seatInTheRoomBetter[seedIndex + distance].id &&
          !triedSeatIds.includes(seatInTheRoomBetter[seedIndex + distance].id)
        ) {
          return seatInTheRoomBetter[seedIndex + distance].id
      ...
      for (let index = 0; index < seatInTheRoomPoor.length; index++) {
        if (
          !!seatInTheRoomPoor[index].id &&
          !triedSeatIds.includes(seatInTheRoomPoor[index].id)
        ) {
    ...
    searchSeatsByTime (buildingId, roomId, date, startTime, endTime, token) {
      libraryRestApi
        .SearchSeat(buildingId, roomId, date, startTime, endTime, token)
        .then((response) => {
        ...
        .then((response) => {
          if (response.data.status) {
            ...
            if (response.data.data.seats) {
              for (var key in response.data.data.seats) {
                if (response.data.data.seats[key].status === 'FREE') {
                  this.seatsForSelectIsAvailable.push(
                    response.data.data.seats[key].id
                  )
                }
              }
            }
    ...
    getSeatColor (seatId) {
      if (this.seatsForSelectIsAvailable === null) {
        ...
        this.seatsForSelectIsAvailable.length === 0 ||
        !this.seatsForSelectIsAvailable.includes(seatId)
      ) {
        return 'red'
      } else {
        return 'rgb(0, 204, 0)'
