<template>
  <div class="authen">
    ...
    <div
      class="auth-button"
      :class="{ checked: authInfo.useListForAuth }"
      @click="useListBtnClicked()"
    >
      <i class="button-icon">
        <svg
          class="button-icon"
    ...
    <div
      class="auth-button"
      :class="{ checked: authInfo.haveStaredRepo, disable: ghBtnDisable }"
      @click="ghAuthBtnClicked()"
    >
      <i class="button-icon">
      ...
      <div class="gap"></div>
      <span v-if="authInfo.haveStaredRepo" class="text"
        >[ √ ]&nbsp;&nbsp;GitHub 已授权</span
      >
      <span v-else class="text">{{ ghBtnMessage }}</span>
    </div>
    <span v-if="socketFailMessage" class="socket-err-msg">{{
      socketFailMessage
    }}</span>
    <div class="help-link-wrap">
      <a
        ...
        @click="
          $openLink(
            'https://home.cs-tao.cc/whu-library-seat/specification/auth.html'
          )
        "
        >了解授权方法</a
      >
    ...
    >
      ...
      <a
        class="help-link"
        href="javascript:void(0)"
        @click="$openLink('https://github.com/settings/tokens/new')"
        >Developer settings 页面</a
      >
      <span>手动申请 token 后粘贴到下方，以继续获取本软件授权</span>
      <br />
      <span class="tip"
        >Tip: 进入上方链接，输入任意 Note，生成并复制 token 即可，无需任何
        Scope</span
      >
      <el-input
        v-model="inputToken"
        placeholder="Ctrl + V  to paste your token"
        style="margin-top: 8px"
      />
      <span slot="footer" class="dialog-footer">
        <el-button
          class="comfirm-button"
          @click="onComfirmInputTokenDialogYes()"
          >确 定</el-button
        >
        <el-button class="cancel-button" @click="onComfirmInputTokenDialogNo()"
          >取 消</el-button
        >
      </span>
    </el-dialog>
    ...
    >
      ...
      <i
        ...
        @click="$openLink('https://github.com/CS-Tao/whu-library-seat')"
      >
        <svg
          class="operation-icon"
      ...
      <span>{{
        "&nbsp;图标进入本软件的 GitHub 地址，点星后回到本软件，点击确定按钮即可完成授权"
      }}</span>
      <span slot="footer" class="dialog-footer">
        <el-button class="comfirm-button" @click="onComfirmStarDialogYes()"
          >确 定</el-button
        >
        <el-button class="cancel-button" @click="onComfirmStarDialogNo()"
          >取 消</el-button
        >
      </span>
    </el-dialog>
